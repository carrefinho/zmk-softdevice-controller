...
/* Include the wrapper that aliases nrfx_clock_irq_handler to nrfx_power_clock_irq_handler */
...
/* Note: nrfx_power_clock.h defines nrfx_clock_irq_handler as nrfx_power_clock_irq_handler
 * when only CLOCK is enabled (not POWER). This provides the symbol the clock driver needs.
 */
