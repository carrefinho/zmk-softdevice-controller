...
LOG_MODULE_REGISTER(hci_internal, CONFIG_BT_HCI_DRIVER_LOG_LEVEL);
...
{
	uint16_t opcode = sys_get_le16(cmd_in);
	uint8_t param_len = ((struct bt_hci_cmd_hdr *)cmd_in)->param_len;

	LOG_DBG("opcode=0x%04x param_len=%d", opcode, param_len);
	...
	if (cmd_complete_or_status.occurred
		...
		LOG_ERR("cmd already pending");
		return -NRF_EPERM;
	}

	if ((param_len + BT_HCI_CMD_HDR_SIZE) > HCI_CMD_PACKET_MAX_SIZE) {
		LOG_ERR("cmd too large: %d > %d", param_len + BT_HCI_CMD_HDR_SIZE,
