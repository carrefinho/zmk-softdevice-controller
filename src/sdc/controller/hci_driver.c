...
static int configure_supported_features(void)
{
	...
	/* Channel Sounding mode3/initiator/reflector roles require newer nrfxlib */
#if defined(CONFIG_BT_CTLR_SDC_CS_STEP_MODE3)
...
#if defined(CONFIG_BT_CTLR_SDC_CS_ROLE_INITIATOR_ONLY) || \
	defined(CONFIG_BT_CTLR_SDC_CS_ROLE_BOTH)
...
#if defined(CONFIG_BT_CTLR_SDC_CS_ROLE_REFLECTOR_ONLY) || \
	defined(CONFIG_BT_CTLR_SDC_CS_ROLE_BOTH)
...
#if defined(CONFIG_BT_CTLR_SDC_PERIODIC_SYNC_BUFFER_COUNT)
		cfg.periodic_sync_buffer_cfg.count = CONFIG_BT_CTLR_SDC_PERIODIC_SYNC_BUFFER_COUNT;
#else
		cfg.periodic_sync_buffer_cfg.count = 2; /* Default value */
#endif
		required_memory =
		sdc_cfg_set(SDC_DEFAULT_RESOURCE_CFG_TAG,
...
static int hci_driver_init(const struct device *dev)
{
	...
	if (!mpsl_is_initialized()) {
		LOG_ERR("MPSL not initialized");
		return -EAGAIN;
	}

	err = sdc_init(sdc_assertion_handler);
	if (err) {
		LOG_ERR("sdc_init failed: %d", err);
		return err;
	}
	LOG_INF("SoftDevice Controller initialized");

	err = configure_supported_features();
	if (err) {
		LOG_ERR("configure_supported_features failed: %d", err);
		return err;
	}
	LOG_DBG("configure_supported_features ok");

	err = configure_memory_usage();
	if (err) {
		LOG_ERR("configure_memory_usage failed: %d", err);
		return err;
	}
	LOG_DBG("configure_memory_usage ok");

	LOG_INF("hci_driver_init complete");
	return err;
}
